function [features, empmean, CRFDATA] = computeSceneZnegpairwisefeature(features, empmean, imlist, CRFDATA, ALLPATHS, train)
...
potentialfile = fullfile(ALLPATHS.SCENE_PATH, 'potential', 'scenepotentialsbinary.mat');
data = load(potentialfile);
pot = data.potential;
for i = 1 : NumClasses
   T_P_i = zeros(NumScenes,BinaryStates);
   unoccurr = (pot(i, :) == 0);    % for which scenes class i never appears
   ind = find(unoccurr);
   if numel(ind)
      T_P_i(ind, 2) = -10000;
   end;
